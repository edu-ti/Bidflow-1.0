...
class MonitorPE
{
    ...
    public function run()
    {
        ...
        echo "--> Modo: MONITORAMENTO ONLINE (cURL)\n";
    ...
    private function fetchContent()
    {
        ...
        if (empty($url)) {
            echo "--> [AVISO] URL do Portal não configurada. Usando MOCK.\n";
        ...
        // Cookie de Sessão
        $cookie = $this->config['portal_cookie'] ?? '';
        if (!empty($cookie)) {
            // Se o usuário colou o header inteiro 'Cookie: ...', removemos o prefixo, senão usamos como está
            $cookieVal = str_ireplace('Cookie:', '', $cookie);
            $cookieVal = trim($cookieVal);
            curl_setopt($ch, CURLOPT_COOKIE, $cookieVal);
        }
        ...
        if ($httpCode != 200 || empty($output)) {
            echo "--> [ERRO] Falha na requisição. HTTP Code: $httpCode. Erro: $curlError\n";
            // Em caso de erro, retornamos vazio para não processar lixo
            return "";
        }

        echo "--> [SUCESSO] Conteúdo baixado. Tamanho: " . strlen($output) . " bytes.\n";
        return $output;
    }
