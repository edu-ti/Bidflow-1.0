...
// Se houver feedback salvo na sessão, carrega-o para exibição
if (isset($_SESSION['monitor_feedback'])) {
    $msgFeedback = $_SESSION['monitor_feedback'];
    unset($_SESSION['monitor_feedback']);
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['run_monitor'])) {
    // Ao invés de usar o CLI, vamos incluir e instanciar aqui
    require_once 'monitor_pe.php';

    // Captura toda saída gerada pela classe no buffer
    ob_start();
    $monitor = new MonitorPE();
    $monitor->run();
    $outputString = ob_get_clean();

    // Converte Newlines do console para <br>
    $_SESSION['monitor_feedback'] = nl2br(htmlspecialchars($outputString));

    // Força recarregar logs para limpar o POST e trazer fresh data
